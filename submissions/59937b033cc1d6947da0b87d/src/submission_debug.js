...
class Play {
    ...
    play() {
        ...
        ++this.frame;
        if (this.next && !this.compare()) {
            this.log();
            return 'q';
        }
        if (!this.seq_idx)
            return 'q';
        let state = this.seq[--this.seq_idx];
        this.set_next(state);
        this.set_latest();
        return 'urdl '[state.dir];
    }
    ...
    set_latest() {
        this.latest = [];
        for (let y = 0; y < this.h; ++y)
            this.latest.push(this.screen[y]);
    }

    compare() {
        for (let y = 0; y < this.h; ++y) {
            for (let x = 0; x < this.w; ++x) {
                let c1 = this.screen[y][x];
                let c2 = this.next[y][x];
                if ('\\|-'.includes(c1))
                    c1 = '/';
                if (c2 === 'X')
                    c2 = '*';
        ...
        return this.get_score() === this.score;
    }

    log() {
        console.error("\nScreen actual/expected/latest:");
        for (let y = 0; y < this.h; ++y)
            console.error(this.screen[y] + ' ' + this.next[y] + (this.latest ? ' ' + this.latest[y] : ''));
        console.error("Score actual/expected: " + this.get_score() + ' ' + this.score);
        this.log_units(0, 'This:');
        this.log_units(1, 'Latest:');
        this.log_units(2, 'Before latest:');
    }

    log_units(idx, label) {
        let units = this.units[idx];
        let msg = label;
        for (let k in units) {
            if (units.hasOwnProperty(k)) {
                let u = units[k];
                msg += ' ' + k + ' ' + (u.is_butterfly() ? ('b-' + u.dir) : ('f-' + u.falling));
            }
        }
        console.error(msg);
    }
    ...
    get_score() {
        return parseInt(this.screen[this.h].substr(8, 6));
    }
}
