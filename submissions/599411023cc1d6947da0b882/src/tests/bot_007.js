...
//		console.log("check ", group, " at pos [", pos.x, ",", pos.y, "]");
	...
	screen.find_way = function ( from, to ) {
		let way = screen._find_way(from, to, false, 20);
	...
	screen._find_way  = function ( from, to, way, depth = 20 ) {
		if ( depth < 0 ) {
			return false;
		}
...
let bot = (function () {
	...
	let FLY_TARGET_TTL = 90;
	let ALMAZ_TARGET_TTL = 90;
		...
		*/
		...
		if ( target === false ) {
			...
			for ( let i=0; i<list.length; i++ ) {
				...
				if (
					!target
					|| player.distance(target.pos) > player.distance(list[i])
				) {
					target = {
						type: 'almaz',
		...
		if ( target ) {
			if ( !target.debug ) {
				target.debug = function () {
					...
					switch ( target.type ) {
						case 'fly':
							return 'fly#'+target.index+' ' + target.pos.debug() + target.pos.direction + ' ' + target.ttl;
						case 'almaz':
		...
		if ( target && (!way || target.type == 'fly') ) {
			...
			if ( !way && target.type == 'almaz' && target.ttl > 10 ) {
				target.ttl = 10;
				target.pos.ignore = true;
				almaz_list[target.index].ignore = true;
			}
		}
	...
	$.calc_weight = function ( move ) {
		let direction	= move[0];
		// не меняем вес для выхода из игры
		if ( direction == 'q' ) {
			return move;
		}
...
//		}
		...
		if ( target ) {
			...
			switch ( target.type ) {
				case 'fly':
					// охота на бабочек
					// занять позицию "сверху"
					if ( target.pos.y > player.y ) {
						weight += 2;
					}
					if ( target.pos.y > to.y ) {
						weight += 2;
					}
					// сблизиться по X
					if ( Math.abs(target.pos.x - to.x) < Math.abs(target.pos.x - player.x) ) {
						weight += 2;
					}
					// ловушки
					if (
						   ( screen.is('falling', to.u(1)) && screen.is('empty', to.d()) )
						|| ( screen.is('falling', to.u(2)) && screen.is('empty', to.d()) && screen.at(to.u()) == ':' )
					) {
						// не ломаем если цель далеко по X
						if ( screen.is(':', to) ) {
							weight -= Math.abs(to.x - target.pos.x)*2;
						}
						if (
							to.x == target.pos.x
							&& ( target.pos.direction == 'u' || screen.ways(target.pos) == 'ud' )
						) {
							weight += 20;
						}
					}
					if (
						...
						&& screen.is('O', to)
						&& ( screen.is('empty', to[direction]().d()) || screen.is('fly', to[direction]().d()) )
					) {
						weight += 20;
					}
					if (
						...
						&& (
							(
								screen.is('O', to.l()) && screen.is('empty', to.l(2))
								&& Math.abs(to.l().x - target.pos.x) < Math.abs(player.x - target.pos.x)
							)
							|| (
								screen.is('O', to.r()) && screen.is('empty', to.r(2))
								&& Math.abs(to.r().x - target.pos.x) < Math.abs(player.x - target.pos.x)
							)
						)
					) {
						weight += 10;
