...
let bot = (function () {
	...
	const TARGET_FLY_TTL = 90;
	const TARGET_ALMAZ_TTL = 90;
		...
		if ( target && (!way || target.type == 'fly') ) {
			...
			if ( !way && target.type == 'almaz' ) {
				if (
					...
					target.ttl = 5;
				} else if ( target.ttl > TARGET_ALMAZ_TTL*.5 ) {
					target.ttl = Math.floor(TARGET_ALMAZ_TTL*.5);
				}
			}
	...
	$.get_possible_moves = function ( player, screen ) {
		...
		return [
			...
			if (
				' *:'.includes(val)
				|| (val == 'O' && direction == 'l' && screen.is('empty', pos.l()))
				|| (val == 'O' && direction == 'r' && screen.is('empty', pos.r()))
				|| (val == 'O' && screen.is('empty', pos.d()))
			) {
				return true;
			}
			return false;
			...
			switch ( target.type ) {
				case 'fly':
					...
					*/
					...
					// traps
					if (
						target.pos.y > player.y				// игрок над целью
						&& Math.abs(target.pos.x-player.x) < 2	// в пределах +-1 клетка по X
					) {
						// игрок под камнем
						if ( screen.is('falling', player.u()) ) {
							if ( direction == 'u' ) {
								// уменьшить шанс испортить ловушку алмазом
								weight += -10;
							}
							if (
								// цель двигается вверх
								target.pos.direction == 'u'
								// или цель изменит направление
								|| ( target.pos.direction && !screen.is('empty', target.pos[target.pos.direction]()) )
							) {
								// увеличить ценность шага в сторону
								if ( direction == 'l' || direction == 'r' ) {
									weight += 20;
								}
							} else if (
								direction == ' '
								&& screen.is('empty', player.d())
								&& screen.is('empty', player.d(2))
						...
						if (
							screen.is('empty', player.d())
							&& player.distance(target.pos) < 5
						) {
							if ( direction == 'd' ) {
								weight += -10;
							} else {
								if (
									( screen.is('O*', player.l()) && screen.is('O*', player.l().u()) )
									|| ( screen.is('O*', player.l().u()) && screen.is('O*', player.l().u(2)) )
									|| ( screen.is('O*', player.l()) && screen.is('O*', player.l().d()) )
								) {
									switch ( direction ) {
										case 'u':
											weight += 10;
						...
						for ( let dir of ['l', 'r'] ) {
							if (
								screen.is('falling', player[dir]().u(2))
								&& Math.abs(player[dir]().x - target.pos.x) < 2
								&& screen.is(':*', player[dir]().u())
								&& (
									target.pos.direction == 'u'
									|| ( target.pos.direction == dir && !screen.is('empty', target.pos[target.pos.direction]()) )
								)
								&& target.pos.x == player.x
							) {
								switch ( direction ) {
									case dir:
										weight += 30;
				...
				case 'almaz':
					...
					if (
						(screen.is('O', player.u()) || (screen.is('empty', player.u()) && screen.is('O', player.u(2))))
						&& screen.ways(to) == ''
					) {
						// в тупик
						weight += -20;
