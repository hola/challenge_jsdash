...
function starlist2(screen, player, grid) {
    let starList = [];
    for (let y = 0; y < screen.length; y++) {
        let row = screen[y];
        for (let x = 0; x < row.length; x++) {
...
exports.play = function* (screen) {
    ...
    while (true) {
        ...
        {
            let bitMap2 = bitmap2(screen,player);
            let dots = starlist2(screen,player,bitMap2);
            if(dots.length)
            {
                
                let graph = new Graph(bitMap2);
                let start = graph.grid[player.y][player.x];
                let end = graph.grid[dots[0].y][dots[0].x];
                let result = astar.search(graph, start, end).map(a=>{return {x:a.y,y:a.x}});
