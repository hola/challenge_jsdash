...
function getMovesOrderToCheck(exclude): string[] {
  return 'urdl'.replace(exclude, '').split('');
}

export function char2dir(c) {
  switch (c){
    case 'u': return UP;
    case 'd': return DOWN;
    case 'r': return RIGHT;
...
export class Point implements Coordinates {
  x: number;
  y: number;
  static containsCoordinates(collection: Coordinates[], neededCoordinates: Coordinates): boolean {
    return !!collection.find(c => c.x === neededCoordinates.x && c.y === neededCoordinates.y);
  }
  static getMove(fromCoordinates: Coordinates, toCoordinates: Coordinates): string {
  ...
  static getNextCoordinates(currentCoordinates: Coordinates, moveTo: string): Coordinates {
    if (moveTo === 'l') {
  ...
  static getClosestPath(coordinates: string[][]): string[] {
   ...
   */
  static allCrest(coordinates: Coordinates): Coordinates[] {
    return [
  ...
  static all(coordinates: Coordinates): Coordinates[] {
    return [
  ...
  static up(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x, y: coordinates.y - 1};
  }
  static down(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x, y: coordinates.y + 1};
  }
  static left(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x - 1, y: coordinates.y};
  }
  static right(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x + 1, y: coordinates.y};
  }
  static upLeft(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x - 1, y: coordinates.y - 1};
  }
  static downLeft(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x - 1, y: coordinates.y + 1};
  }
  static upRight(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x + 1, y: coordinates.y - 1};
  }
  static downRight(coordinates: Coordinates): Coordinates {
    return {x: coordinates.x + 1, y: coordinates.y + 1};
  }
  constructor(x, y){
    this.x = x;
...
export class SteelWall extends Thing {
  clone(world): SteelWall {
...
export class BrickWall extends Thing {
  clone(world): BrickWall {
...
export class Dirt extends Thing {
  clone(world): Dirt {
...
export class LooseThing extends Thing {
  falling: boolean;
...
export class Boulder extends LooseThing {
  static counter = 0;
  id = 0;
  clone(world): Boulder {
...
export class Diamond extends LooseThing {
  clone(world): Boulder {
...
export class ButterflyT extends Thing {
  dir: any;
  alive: boolean;
...
export class World {
  ...
  score: number;
  streak: number;
  streak_expiry: number;
  streak_message: string;
  streaks: number;
  longest_streak: number;
  diamonds_collected: number;
  butterflies_killed: number;
  cells: any[][];
  scored_expiry: any;
  screen: Screen = [];
  lastBreaksPosition: string;
  ignoreDiamonds = false;
  ignoreDeadEnds = false;
  currentTarget: Coordinates;
  movesQueue: string[] = [];
  lowPriorityDiamonds: LowPriorityDiamond[];
  nextLeft: World;
  nextRight: World;
  nextUp: World;
  nextDown: World;
  ...
  static getCoordinatesByPath(currentCoordinates: Coordinates, path: string[]): Coordinates {
  ...
  cloneAndMove(moveTo: string): World {
  ...
  *[Symbol.iterator]() {
  ...
  control(c) {
    ...
    this.player.movesOrder.push({...this.player.point});
  }
  is_playable(): boolean {
  ...
  checkDeadEnd(currentCoordinates: Coordinates, moveTo: string, ignoreDiamonds?: boolean): boolean {
    ...
    function echo(coordinates: Coordinates) {
  ...
  mapCleanup(): string {
    ...
    if (this.player.isStuck()) {
      this.selectCurrentFallen([{...this.currentTarget}]);
  ...
  selectCurrentTarget(ignored = []) {
    ...
    let paths: string[][] = allDiamonds.map(diamond => this.getMovesTo(diamond)).filter(p => !(p.length === 1 && p[0] === ''));
    if (!paths.length) {
      this.ignoreDeadEnds = true;
      allDiamonds = allDiamondsCopy;
  ...
  getMovesTo(targetCoordinates: Coordinates): string[] {
    const screen = this.screen;
  ...
  getCoordinatesForAllOfType(type: string, ignored: Coordinates[] = []): Coordinates[] {
    const screen = this.screen;
    let coordinates = [];
  ...
  calculateBreaksPositions(withDiamonds = false, withPlayer = false): string {
    const screen = this.screen;
    let res = {};
    for (let y = 0; y < screen.length; y++) {
      let row = screen[y];
  ...
  getCoordinatesForAllFallenDiamonds(ignored: Coordinates[] = []): Coordinates[] {
    const screen = this.screen;
    let coordinates = [];
    for (let y = 0; y <= screen.length / 2; y++) {
      ...
      for (let x = 0; x < row.length; x++) {
        if (
          FieldIs.FallenDiamond(screen, {x, y}) ||
  ...
  getCoordinatesForAllFallenBoulders(ignored: Coordinates[] = []): Coordinates[] {
    const screen = this.screen;
    let coordinates = [];
    for (let y = 0; y <= screen.length / 2; y++) {
      let row = screen[y];
      for (let x = 0; x < row.length; x++) {
        if (
          FieldIs.FallenBoulder(screen, {x, y}) ||
  ...
  private _canMoveTo(currentCoordinates: Coordinates, moveTo: string): boolean {
    ...
    if (butterflyNearBy) { // i don't think it's accessible. `cloneNext.player.alive` should be false because of explosion
      console.log('####', this.frame, moveTo, '\t\t\t\t\t\t\t');
      return false;
    }
  ...
  canMoveLeft(currentCoordinates: Coordinates): boolean {
    return this._canMoveTo(currentCoordinates, 'l');
  }
  canMoveRight(currentCoordinates: Coordinates): boolean {
    return this._canMoveTo(currentCoordinates, 'r');
  }
  canMoveUp(currentCoordinates: Coordinates): boolean {
    return this._canMoveTo(currentCoordinates, 'u');
  }
  canMoveDown(currentCoordinates: Coordinates): boolean {
    return this._canMoveTo(currentCoordinates, 'd');
  }
  canStay(currentCoordinates: Coordinates): boolean {
    const clone = this.getNextMoveClone('');
    const cloneNext = clone.cloneAndMove('');
    if (!clone.player.alive || !cloneNext.player.alive) {
      console.log('@@@@', this.frame, 'stay');
      return false;
    }
    const directions = 'lurd';
    for (let i = 0; i < directions.length; i++) {
  ...
  doNextMove(currentCoordinates: Coordinates, moveTo: string): string {
    console.log(this.frame, 'current: ', currentCoordinates, moveTo, this.pathFinder, this.stayCounter, JSON.stringify(this.currentTarget));
...
export function from_ascii(rows, opt = {frames: 1200, fps: 10}) {
...
export class FieldIs {
  static is(screen: Screen, coordinates: Coordinates, needed): boolean {
    if (!coordinates) {
      return false;
    }
   ...
   * Field in a Diamond ('*')
   */
  static Diamond(screen: Screen, coordinates: Coordinates): boolean {
   ...
   */
  static Boulder(screen: Screen, coordinates: Coordinates): boolean {
   ...
   * Field is a Dirt (':')
   */
  static Dirt(screen: Screen, coordinates: Coordinates): boolean {
    return FieldIs.is(screen, coordinates, ':');
  }
   ...
   */
  static Blank(screen: Screen, coordinates: Coordinates): boolean {
    return FieldIs.is(screen, coordinates, ' ');
  }
   ...
   */
  static Butterfly(screen: Screen, coordinates: Coordinates): boolean {
    return FieldIs.is(screen, coordinates, ['-', '/', '\\', '|']);
  }
   ...
   */
  static Passable(screen: Screen, coordinates: Coordinates): boolean {
   ...
   */
  static Dangerous(screen: Screen, coordinates: Coordinates): boolean {
   ...
   */
  static Wall(screen: Screen, coordinates: Coordinates): boolean {
    return FieldIs.is(screen, coordinates, ['#', '+']);
  }
   ...
   * Field is a Player ('A')
   */
  static Player(screen: Screen, coordinates: Coordinates): boolean {
   ...
   */
  static CauseToFallDiamond(screen, coordinates): boolean {
   ...
   */
  static CauseToFallBoulder(screen, coordinates): boolean {
   ...
   */
  static FallenBoulder(screen: Screen, coordinates: Coordinates): boolean {
   ...
   */
  static RollRightBoulder(screen: Screen, coordinates: Coordinates): boolean {
   ...
   */
  static RollLeftBoulder(screen: Screen, coordinates: Coordinates): boolean {
    const down = Point.down(coordinates);
    const up = Point.up(coordinates);
   ...
   * Field is a Dirt with a Diamond above it
   */
  static FallenDiamond(screen: Screen, coordinates: Coordinates): boolean {
   ...
   * Field is a Dirt with a Diamond on the left (see tests for map examples)
   */
  static RollRightDiamond(screen: Screen, coordinates: Coordinates): boolean {
    const down = Point.down(coordinates);
    const up = Point.up(coordinates);
    const left = Point.left(coordinates);
    const leftDown = Point.left(down);
   ...
   * Field is a Dirt with a Diamond on the right (see tests for map examples)
   */
  static RollLeftDiamond(screen: Screen, coordinates: Coordinates): boolean {
    const down = Point.down(coordinates);
    const up = Point.up(coordinates);
    const right = Point.right(coordinates);
    const rightDown = Point.right(down);
    const rightUp = Point.right(up);
    const scenario0 =
      FieldIs.Diamond(screen, right) &&
      FieldIs.Dirt(screen, coordinates) &&
      (FieldIs.Wall(screen, rightDown) || FieldIs.Dangerous(screen, rightDown)) &&
      (FieldIs.Blank(screen, down) || FieldIs.Player(screen, down) || FieldIs.Dirt(screen, down));
    const scenario1 =
      FieldIs.Diamond(screen, rightUp) &&
      FieldIs.Dirt(screen, coordinates) &&
      (FieldIs.Wall(screen, right) || FieldIs.Dangerous(screen, right)) &&
      (FieldIs.Blank(screen, up) || FieldIs.Player(screen, up) || FieldIs.Dirt(screen, up));
    return scenario0 || scenario1;
...
export function getPlayerCoordinates(screen: Screen): Coordinates {
  for (let y = 0; y < screen.length; y++) {
    let row = screen[y];
    for (let x = 0; x < row.length; x++) {
      if (row[x] === 'A') {
...
export function printScreen(sonar: any[][], fl = false): string {
  let ret = '';
